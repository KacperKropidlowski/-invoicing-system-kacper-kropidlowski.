...
class InFileDatabaseTest extends Specification {
    ...
    Database inFileDatabase = new InFileDatabase(inFileDatabaseTestingPath, idService, filesService, jsonService)
    Database wrongPathDatabase = new InFileDatabase(Path.of("wrongPath"), idService, filesService, jsonService)
    ...
    def "should get invoice by id"() {
        ...
        then:
        result.isPresent()
        result.toString().contains("Invoice(id=1, date=2022-08-25, seller=Company(name=Relax Kebab, taxIdentificationNumber=5212205778, address=aleja Jana Paw³a II 40A, 05-250 Radzymin), buyer=Company(name=AGENCJA MIENIA WOJSKOWEGO, taxIdentificationNumber=5261038122, address=ul. Nowowiejska 26A, 00-911 Warszawa), invoiceEntries=[InvoiceEntry(description=Stumetrowy kebab, price=700, vatValue=161, vatRate=Vat.VAT_23)])") == invoice.toString().contains("Invoice(id=1, date=2022-08-25, seller=Company(name=Relax Kebab, taxIdentificationNumber=5212205778, address=aleja Jana Paw³a II 40A, 05-250 Radzymin), buyer=Company(name=AGENCJA MIENIA WOJSKOWEGO, taxIdentificationNumber=5261038122, address=ul. Nowowiejska 26A, 00-911 Warszawa), invoiceEntries=[InvoiceEntry(description=Stumetrowy kebab, price=700, vatValue=161, vatRate=Vat.VAT_23)])")
    }
    ...
    def "should update invoice"() {
        ...
        then:
        result.toString().contains("id=1, date=2022-08-25, seller=Company(name=AGENCJA MIENIA WOJSKOWEGO, taxIdentificationNumber=5261038122, address=ul. Nowowiejska 26A, 00-911 Warszawa)")
    }
